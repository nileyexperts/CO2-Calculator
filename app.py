...
def reset_form(max_segments: int = MAX_SEGMENTS):
    ...
    for i in range(max_segments):
        widget_keys.extend([f"origin_input_{i}", f"origin_select_{i}", f"dest_input_{
                           i}", f"dest_select_{i}", f"mode_{i}", f"weight_{i}"])
    for k in widget_keys + ["segments",
                            "osrm_base_url",
                            "weight_0",
                            "dossier_transport"]:
...
with st.expander("⚙️ Paramètres, facteurs d'émission & OSRM"):
    ...
    weight_mode = st.radio(
        "Mode de gestion du poids :", [
            default_mode_label, "Poids par segment"], horizontal=False)
    ...
    unit = st.radio(
        "Unite de saisie du poids", [
            "kg", "tonnes"], index=0, horizontal=True)
    ...
    osrm_default = st.session_state.get(
...
with st.expander("🎯 Apparence de la carte (points & logos)"):
    ...
    if dynamic_radius:
        radius_m = st.slider("Rayon des points (mètres)",
    ...
    icon_size_px = st.slider(
        "Taille des logos de segment (pixels)", 16, 64, 28, 2)
...
for i in range(1, len(st.session_state.segments)):
    ...
    if prev.get("dest_sel") and not cur.get(
            "origin_raw") and not cur.get("origin_sel"):
...
for i in range(len(st.session_state.segments)):
    ...
    with c1:
        origin_raw = st.text_input(
            f"Origine du segment {
                i+1}",
            value=st.session_state.segments[i]["origin_raw"],
            key=f"origin_input_{i}")
        origin_suggestions = geocode_cached(
            origin_raw, limit=5) if origin_raw else []
        origin_options = [r['formatted']
                          for r in origin_suggestions] if origin_suggestions else []
        origin_sel = st.selectbox(
            "Suggestions pour l'origine",
            origin_options or ["-"],
            index=0,
    ...
    with c2:
        dest_raw = st.text_input(
            f"Destination du segment {
                i+1}",
            value=st.session_state.segments[i]["dest_raw"],
            key=f"dest_input_{i}")
        dest_suggestions = geocode_cached(
            dest_raw, limit=5) if dest_raw else []
        dest_options = [r['formatted']
                        for r in dest_suggestions] if dest_suggestions else []
    ...
    mode = st.selectbox(
        f"Mode de transport du segment {
            i+1}",
        list(
            factors.keys()),
        index=list(
            factors.keys()).index(
            st.session_state.segments[i]["mode"]) if st.session_state.segments[i]["mode"] in factors else 0,
        key=f"mode_{i}")
...
st.markdown("")
if st.button("➕ Ajouter un segment à la fin", key="add_at_end",
             disabled=len(st.session_state.segments) >= MAX_SEGMENTS):
...
def mode_to_category(mode_str: str) -> str:
    ...
    if "routier" in s:
        return "routier"
    if "aerien" in s or "aérien" in s:
        return "aerien"
    if "maritime" in s or "mer" in s or "bateau" in s:
        return "maritime"
    if "ferroviaire" in s or "train" in s:
        return "ferroviaire"
    if "road" in s or "truck" in s:
        return "routier"
    if "air" in s or "plane" in s:
        return "aerien"
    if "sea" in s or "ship" in s:
        return "maritime"
    if "rail" in s:
        return "ferroviaire"
    return "routier"
...
def midpoint_on_path(route_coords, lon_o, lat_o, lon_d, lat_d):
    if route_coords and isinstance(
            route_coords,
            list) and len(route_coords) >= 2:
...
if not can_calculate:
    st.warning(
        "Veuillez renseigner le **N° dossier Transport** avant de lancer le calcul.")
...
if st.button(
    ...
    with st.spinner("Calcul en cours..."):
        for idx, seg in enumerate(segments_out, start=1):
            if not seg["origin"] or not seg["destination"]:
                st.warning(
                    f"Segment {idx} : origine/destination manquante(s).")
                continue
            ...
            if "routier" in _normalize_no_diacritics(seg["mode"]):
                try:
                    r = osrm_route(
                        coord1,
                        coord2,
                        st.session_state["osrm_base_url"],
                        overview="full")
                    distance_km = r["distance_km"]
                    route_coords = r["coords"]
                except Exception as e:
                    st.warning(f"Segment {idx}: OSRM indisponible ({
                               e}). Distance à vol d'oiseau utilisée.")
    ...
    if rows:
        ...
        st.success(
            f"✅ {
                len(rows)} segment(s) calculé(s) • Distance totale : {
                total_distance:.1f} km • Émissions totales : {
                total_emissions:.2f} kg CO₂e")

        dossier_val = st.session_state.get("dossier_transport", "")
        if dossier_val:
            st.info(f"**N° dossier Transport :** {dossier_val}")

        st.dataframe(df[["Segment",
                         "Origine",
                         "Destination",
                         "Mode",
                         "Distance (km)",
                         f"Poids ({unit})",
                         "Facteur (kg CO₂e/t.km)",
                         "Émissions (kg CO₂e)"]],
        ...
        route_paths = []
        for r in rows:
            if "routier" in _normalize_no_diacritics(
                    r["Mode"]) and r.get("route_coords"):
                route_paths.append({"path": r["route_coords"], "name": f"Segment {
                                   r['Segment']} - {r['Mode']}"})
        ...
        for r in rows:
            if not (
                "routier" in _normalize_no_diacritics(
                    r["Mode"]) and r.get("route_coords")):
                straight_lines.append({"from": [r["lon_o"], r["lat_o"]], "to": [
                                      r["lon_d"], r["lat_d"]], "name": f"Segment {r['Segment']} - {r['Mode']}"})
        if straight_lines:
            layers.append(
                pdk.Layer(
                    "LineLayer",
        ...
        points, labels = [], []
        for r in rows:
            points.append({"position": [r["lon_o"], r["lat_o"]], "name": f"S{
                          r['Segment']} • Origine", "color": [0, 122, 255, 220]})
            labels.append({"position": [r["lon_o"], r["lat_o"]], "text": f"S{
                          r['Segment']} O", "color": [0, 122, 255, 255]})
            points.append({"position": [r["lon_d"], r["lat_d"]], "name": f"S{
                          r['Segment']} • Destination", "color": [220, 66, 66, 220]})
            labels.append({"position": [r["lon_d"], r["lat_d"]], "text": f"S{
                          r['Segment']} D", "color": [220, 66, 66, 255]})
        ...
        def midpoint_on_path(route_coords, lon_o, lat_o, lon_d, lat_d):
            if route_coords and isinstance(
                    route_coords, list) and len(route_coords) >= 2:
                idx = len(route_coords) // 2
                pt = route_coords[idx]
                return [float(pt[0]), float(pt[1])]
            return [(lon_o + lon_d) / 2.0, (lat_o + lat_d) / 2.0]
        ...
        for r in rows:
            ...
            lon_mid, lat_mid = midpoint_on_path(
                r.get("route_coords"), r["lon_o"], r["lat_o"], r["lon_d"], r["lat_d"])
            icons.append({"position": [lon_mid,
                                       lat_mid],
        ...
        all_lats.extend([r["lat_o"]
                        for r in rows] + [r["lat_d"] for r in rows])
        all_lons.extend([r["lon_o"]
                        for r in rows] + [r["lon_d"] for r in rows])

        view = _compute_auto_view(
            all_lats, all_lons, viewport_px=(
                900, 600), padding_px=80)
    ...
    safe_suffix = "".join(c if (c.isalnum() or c in "-_")
                          else "_" for c in raw_suffix)
    ...
    st.download_button(
        ...
        st.info(
            "Aucun segment valide n’a été calculé. Vérifiez les entrées ou les sélections.")
