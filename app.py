...
def fit_extent_to_aspect(min_lon, max_lon, min_lat, max_lat, target_aspect_w_over_h):
    # Ajuste l'emprise (en degres) pour correspondre au ratio cible du cadre.
    # Elargit uniquement (ne coupe pas). Evite toute docstring multi-lignes.
    ...
    try:
        ...
        if not use_cartopy:
            ...
            lats = np.arange(
                ...
                math.ceil(max_lat / lat_step) * lat_step + 1e-9,
                lat_step
            )
            lons = np.arange(
                math.floor(min_lon / lon_step) * lon_step,
                math.ceil(max_lon / lon_step) * lon_step + 1e-9,
                lon_step
            )
            for y in lats:
...
# Parametres
# =========================
with st.expander("Parametres, facteurs d'emission & OSRM"):
...
def select_mode_with_icons(segment_index: int, current_value: str) -> str:
    ...
    for i, m in enumerate(MODE_META):
        with cols[i]:
            ...
            new_checks.append(
                st.checkbox(" ", key=f"mode_chk_{segment_index}_{m['id']}", value=(i==checked_index), help=f"Sélectionner {m['label']}")
            )
...
if st.button("Calculer l'empreinte carbone totale", disabled=not can_calculate):
    ...
    if rows:
        ...
        for r in rows:
            ...
            if not url:
                continue
            # Point milieu simple (ou milieu de polyline pour routier)
